...
class NCSNpp(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.num_frames_cond = num_frames_cond = config.data.num_frames_cond + getattr(config.data, "num_frames_future",
                                                                                       0)
        ...
        if conditional:
            ...
            if self.cond_emb:
                modules.append(torch.nn.Embedding(num_embeddings=2,
                                                  embedding_dim=nf // 2))  # makes it 8 times smaller (16 if ngf=32) since it should be small because there are only two possible values:
                temb_dim += nf // 2
        ...
        if self.pseudo3d:
            conv3x3 = functools.partial(layers3d.ddpm_conv3x3_pseudo3d, n_frames=self.n_frames,
                                        act=self.act)  # Activation here as per https://arxiv.org/abs/1809.04096
        ...
        modules.append(layerspp.get_act_norm(act=act, act_emb=act, norm='group', ch=in_ch, is3d=self.is3d,
                                             n_frames=self.num_frames))
        modules.append(conv3x3_last(in_ch, channels * self.num_frames, init_scale=init_scale))

        self.all_modules = nn.ModuleList(modules)

    def forward(self, x, time_cond, actions, cond=None, cond_mask=None):
        # timestep/noise_level embedding; only for continuous training
        ...
        if getattr(self.config.model, 'output_all_frames',
                   False) and cond is not None:  # we only keep the non-cond images (but we could use them eventually)
            _, h = torch.split(h, [self.num_frames_cond * self.config.data.channels,
...
class SPADE_NCSNpp(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.pseudo3d:
            conv3x3 = functools.partial(layers3d.ddpm_conv3x3_pseudo3d, n_frames=self.num_frames,
                                        act=self.act)  # Activation here as per https://arxiv.org/abs/1809.04096
        ...
        modules.append(layerspp.get_act_norm(act=act, act_emb=act, norm='spade', ch=in_ch, is3d=self.is3d,
                                             n_frames=self.num_frames, num_frames_cond=num_frames_cond,
    ...
    def forward(self, x, time_cond, cond=None, cond_mask=None):
        ...
        if self.is3d:  # B, N*C, H, W -> B, C*N, H, W : subtle but important difference!
            ...
            if cond is not None:
                B, NC, H, W = cond.shape
                CN = NC
                cond = cond.reshape(B, self.num_frames_cond, self.channels, H, W).permute(0, 2, 1, 3, 4).reshape(B, CN,
                                                                                                                 H, W)
